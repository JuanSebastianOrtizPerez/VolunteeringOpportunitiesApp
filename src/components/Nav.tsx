...
import { Search, Clock, LayoutDashboard, LogOut, Heart, Plus, Users, ChevronDown } from 'lucide-react'
        ...
        <div className="max-w-6xl mx-auto px-6 h-9 flex items-center justify-end">
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-medium text-green-100 hover:text-white transition-colors py-1 px-2 rounded hover:bg-green-600"
            >
              <Users size={13} />
            ...
            {creatorsOpen && (
              <div className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 w-52 z-50">
                ...
                {CREATOR_SCREENS.map(c => (
                  <button
                    key={c.screen}
                    onClick={() => { onNavigate(c.screen); setCreatorsOpen(false) }}
                    className={`w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors flex items-center gap-2 ${screen === c.screen ? 'text-green-700 font-medium' : ''}`}
                  >
...
function NavBtn({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${active ? 'bg-green-50 text-green-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
    >
      {icon}
