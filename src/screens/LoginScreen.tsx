...
export default function LoginScreen({ onNavigate }: Props) {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    // AuthContext will handle redirect via profile load
  }
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-green-600 text-white py-3 rounded-xl font-semibold text-sm hover:bg-green-700 transition disabled:opacity-60 disabled:cursor-not-allowed mt-2"
            >
          ...
          <div className="mt-6 pt-6 border-t border-gray-100 text-center space-y-2">
            <p className="text-sm text-gray-500">
              New volunteer?{' '}
              <button onClick={() => onNavigate('signup-volunteer')} className="text-green-600 font-medium hover:text-green-700">
                Create account
              </button>
            </p>
            <p className="text-sm text-gray-500">
              Posting opportunities?{' '}
              <button onClick={() => onNavigate('signup-company')} className="text-green-600 font-medium hover:text-green-700">
                Register your org
              </button>
            </p>
          </div>
