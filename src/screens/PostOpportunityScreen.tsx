...
const STOCK_IMAGES: Record<Exclude<Category, 'All'>, string> = {
...
export default function PostOpportunityScreen({ onNavigate }: Props) {
  ...
  const [form, setForm] = useState({
    title: '',
    category: 'Community' as Exclude<Category, 'All'>,
    location: '',
    commitment: '',
    spots: '10',
    description: '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.from('events').insert({
      ...
      skills,
      image: STOCK_IMAGES[form.category],
    })
    ...
    if (err) {
      ...
      setSuccess(true)
      setTimeout(() => onNavigate('company-dash'), 2500)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-100 px-6 pt-8 pb-6">
        <div className="max-w-2xl mx-auto">
          <button onClick={() => onNavigate('company-dash')} className="flex items-center gap-2 text-gray-500 hover:text-gray-700 text-sm mb-4 transition-colors">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Opportunity title</label>
            <input
              value={form.title} onChange={set('title')} required
              placeholder="e.g. Youth Coding Instructor"
          ...
          <div>
            ...
            <select
              ...
              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-400 transition bg-white"
            >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Commitment</label>
              <input
                value={form.commitment} onChange={set('commitment')} required
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Available spots</label>
            <input
              type="number" min="1" value={form.spots} onChange={set('spots')} required
              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-400 transition"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Describe the role, what volunteers will do, and who you're looking for…"
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <input
                ...
                className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-400 transition"
              />
              <button type="button" onClick={addSkill} className="bg-gray-100 hover:bg-gray-200 text-gray-600 px-3.5 py-2.5 rounded-xl transition">
                <Plus size={16} />
              </button>
            </div>
            {skills.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {skills.map(s => (
                  <span key={s} className="inline-flex items-center gap-1 bg-green-50 text-green-700 text-xs px-3 py-1 rounded-full">
                    {s}
          ...
          <button
            ...
            className="w-full bg-green-600 text-white py-3 rounded-xl font-semibold text-sm hover:bg-green-700 transition disabled:opacity-60"
          >
