        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-2 text-sm text-gray-400">
            {companyProfile?.website && (
              <span className="flex items-center gap-1"><Globe size={13} />{companyProfile.website}</span>
            )}
            <span className="flex items-center gap-1"><Mail size={13} />{companyProfile?.email}</span>
            {companyProfile?.phone && (
              <span className="flex items-center gap-1"><Phone size={13} />{companyProfile.phone}</span>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="py-10 text-center text-gray-400 text-sm px-4">
                <p>No posts yet.</p>
                <button onClick={() => onNavigate('post-opportunity')} className="mt-3 text-green-600 font-medium hover:text-green-700">
              ...
              events.map(evt => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-5 py-3.5 border-b border-gray-50 last:border-0 hover:bg-gray-50/70 transition-colors flex items-center justify-between group ${activeEvent?.id === evt.id ? 'bg-green-50/50' : ''}`}
                  >
                    <div className="flex-1 min-w-0">
                      <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${CATEGORY_COLORS[cat]} mb-1`}>
                        {CATEGORY_ICONS[cat]} {cat}
                      </span>
                      <p className="text-sm font-medium text-gray-900 truncate">{evt.title}</p>
                    </div>
                    <ChevronRight size={14} className="text-gray-400 shrink-0 ml-2 group-hover:text-gray-600 transition-colors" />
                  </button>
                )
              })
            )}
              ...
              <>
                <div className="bg-white rounded-2xl border border-gray-100 p-5">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div>
                      <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${CATEGORY_COLORS[activeEvent.category as Exclude<Category, 'All'>]}`}>
                        {CATEGORY_ICONS[activeEvent.category as Exclude<Category, 'All'>]} {activeEvent.category}
                      </span>
                      <h2 className="text-lg font-semibold text-gray-900 mt-2">{activeEvent.title}</h2>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-2xl font-bold text-green-700">{applicants.length}</p>
                      <p className="text-xs text-gray-400">of {activeEvent.spots} spots</p>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                  <div className="px-5 py-4 border-b border-gray-100">
                    <h3 className="font-semibold text-gray-900 text-sm">
                      Applicants — {loadingApplicants ? '…' : applicants.length} received
                    </h3>
                  </div>
                  {loadingApplicants ? (
                    <div className="p-4 space-y-2">
                    ...
                    <>
                      <div className="grid grid-cols-3 px-5 py-2 text-xs font-medium text-gray-400 border-b border-gray-50">
                        <span>Name</span><span>Email</span><span>Applied</span>
                      </div>
                      {applicants.map(applicant => (
                        <div key={applicant.id} className="border-b border-gray-50 last:border-0">
                          <button
                            ...
                            className="w-full grid grid-cols-3 px-5 py-3.5 text-left hover:bg-gray-50/70 transition-colors"
                          >
                            ...
                            <span className="text-sm text-gray-400 flex items-center gap-1 justify-between">
                              {new Date(applicant.applied_at).toLocaleDateString()}
                              {expandedApplicant === applicant.id ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
                            </span>
                          </button>
                            ...
                            <div className="px-5 pb-4 grid grid-cols-1 sm:grid-cols-2 gap-3 bg-gray-50/50">
                              {applicant.phone && (
                                <div>
                                  ...
                                  <a href={`tel:${applicant.phone}`} className="text-sm text-green-600 hover:underline">{applicant.phone}</a>
                                </div>
                              )}
                              <div>
                                <p className="text-xs text-gray-400 mb-0.5">Availability</p>
                                <p className="text-sm text-gray-700">{applicant.availability}</p>
                              </div>
                              <div className="sm:col-span-2">
                                <p className="text-xs text-gray-400 mb-0.5">Why they want to volunteer</p>
                                <p className="text-sm text-gray-700">{applicant.motivation}</p>
                              </div>
                              <div>
                                <a
                                  href={`mailto:${applicant.email}?subject=Your application to ${activeEvent.title}`}
                                  className="text-sm text-green-600 font-medium hover:text-green-700 hover:underline"
                                >
                                  Reply to {applicant.first_name}
                                </a>
                              </div>
                            </div>
                          )}
