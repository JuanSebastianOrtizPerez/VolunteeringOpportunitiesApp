...
export default function SignupVolunteerScreen({ onNavigate }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: profileErr } = await supabase.from('volunteer_profiles').insert({
      id: data.user.id,
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 w-full max-w-md">
          ...
          <div className="mt-6 pt-6 border-t border-gray-100 text-center space-y-2">
            ...
            <p className="text-sm text-gray-500">
              ...
              <button onClick={() => onNavigate('signup-company')} className="text-green-600 font-medium hover:text-green-700">Register as org</button>
            </p>
          </div>
        </div>
      </div>
